...
for (_, row1), (_, row2) in zip(df_max_inhibition_time.iterrows(), df_intermediate_inhibition_time.iterrows()):
    if (row1["cell_line"], row1["drug"]) == (row2["cell_line"], row2["drug"]):
        traces.append(go.Scatter(
            x=[f"Normalized VUS<br>for {max_inhibition_time}h", f"Normalized VUS<br>for {intermediate_inhibition_time}h" + (" (daily)" if daily else "")],
...
for (_, row) in df_intermediate_inhibition_time.iterrows():
    traces.append(go.Scatter(
        x=[f"Normalized VUS<br>for {intermediate_inhibition_time}h" + (" (daily)" if daily else ""), f"Normalized VUS<br>extrapolated<br>until {max_inhibition_time}h" + (" (daily)" if daily else "")],
...
# left
traces.append(go.Scatter(
    x=[f"Normalized VUS<br>for {max_inhibition_time}h"]*len(df_max_inhibition_time["vus_norm"]),
...
# middle
traces.append(go.Scatter(
    x=[f"Normalized VUS<br>for {intermediate_inhibition_time}h" + (" (daily)" if daily else "")]*len(df_intermediate_inhibition_time["vus_norm"]),
...
# right
traces.append(go.Scatter(
    x=[f"Normalized VUS<br>extrapolated<br>until {max_inhibition_time}h" + (" (daily)" if daily else "")]*len(df_intermediate_inhibition_time["vus_norm_extrapolated"]),
...
layout = go.Layout(
    ...
    yaxis=dict(title="Normalized VUS"),
    width=400,
    height=700,
    showlegend=False
)
